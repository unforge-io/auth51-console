...
function Detail({ s }: { s: Span }) {
  ...
  return (
    <div className="flex flex-col gap-2.5 text-c-text">
      ...
      {claims && (
        <Field label="intent token (decoded)">
          <div className="rounded-md border border-c-border bg-c-bg p-2 font-mono text-[11px] space-y-0.5">
            {['sub', 'aud', 'scope', 'iss', 'cnf', 'checksum', 'computed_checksum', 'exp'].map((k) =>
              claims[k] !== undefined ? (
      ...
      {input !== undefined && (
        <Field label="input">
          <pre className="max-h-40 overflow-auto rounded-md border border-c-border bg-c-bg p-2 text-[10px] font-mono text-c-text-2 whitespace-pre-wrap">{asText(input).slice(0, 4000)}</pre>
        </Field>
      )}
      {output !== undefined && (
        <Field label="output">
          <pre className="max-h-40 overflow-auto rounded-md border border-c-border bg-c-bg p-2 text-[10px] font-mono text-c-text-2 whitespace-pre-wrap">{asText(output).slice(0, 4000)}</pre>
        </Field>
      )}
      ...
      {rawOpen && (
        <div className="rounded-md border border-c-border bg-c-bg p-2 font-mono text-[10px] space-y-0.5 max-h-60 overflow-auto">
          {Object.entries(s.attributes ?? {}).map(([k, v]) => (
            <div key={k} className="break-all"><span className="text-c-text-3">{k}: </span>{asText(v).slice(0, 500)}</div>
          ))}
        </div>
      )}
    </div>
...
export function TraceWaterfall({ spans }: { spans: Span[] }) {
  ...
  return (
    <div className="grid grid-cols-[1fr] gap-3 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
      <div className="rounded-md border border-c-border bg-c-bg overflow-hidden">
        {rows.map((n) => {
          const kind = kindOf(n)
          ...
          return (
            <div key={n.span_id}
              ...
              <div className="flex items-center gap-1 min-w-0" style={{ paddingLeft: n.depth * 12 }}>
                ...
                <span className="truncate text-[11px] font-mono text-c-text-2">{n.name}</span>
              </div>
              <div className="ml-auto flex items-center gap-2 shrink-0 w-[46%]">
      ...
      <div className="rounded-md border border-c-border bg-c-surface-2 p-3">
        {selSpan ? <Detail s={selSpan} /> : <div className="text-[11px] text-c-text-3">Select a span to inspect its tokens, mint/DPoP/RS hops, and attributes.</div>}
      </div>
    </div>
  )
}
