 ...
 *
 * Nodes use custom types defined in FlowNodes.tsx.
 * Edge routing is handled automatically by React Flow.
 *
 * INTERACTION MODEL:
...
function getAct1(): { nodes: Node[]; edges: Edge[] } {
  return {
    nodes: [
      { id: 'studio', type: 'actor', position: { x: 0, y: 80 }, data: { label: 'Studio', sublabel: 'Your Organization', icon: '🏢' } },
      { id: 'producer', type: 'actor', position: { x: 280, y: 80 }, data: { label: 'Line Producer', sublabel: 'Orchestrator Agent', icon: '🎬' } },
      { id: 'vendor', type: 'actor', position: { x: 560, y: 80 }, data: { label: 'Vendor', sublabel: 'Resource / API', icon: '🏪' } },
      { id: 'question', type: 'annotation', position: { x: 220, y: 220 }, data: { label: 'The studio needs crew members to work with vendors — but how do you authorize them while keeping control?', variant: 'brand' } },
      { id: 'next', type: 'check', position: { x: 780, y: 85 }, data: { label: 'Next →', icon: '▶', clickable: true } },
    ],
    edges: [
...
function getAct2(): { nodes: Node[]; edges: Edge[] } {
  return {
    nodes: [
      { id: 'studio', type: 'actor', position: { x: 0, y: 0 }, data: { label: 'Studio', sublabel: 'Organization', icon: '🏢' } },
      { id: 'producer', type: 'actor', position: { x: 250, y: 0 }, data: { label: 'Line Producer', sublabel: 'Agent', icon: '🎬' } },
      { id: 'letter', type: 'artifact', position: { x: 480, y: 0 }, data: { label: 'Letter of Auth', sublabel: 'Bearer Token', icon: '📜' } },
      { id: 'vendor', type: 'actor', position: { x: 700, y: 0 }, data: { label: 'Vendor', sublabel: 'Resource', icon: '🏪' } },
      // Bearer problem branch
      { id: 'bearer-issue', type: 'threat', position: { x: 350, y: 140 }, data: { label: 'Bearer Problem', sublabel: 'Anyone with the letter can use it', icon: '⚠️' } },
      // Auth51 solution
      { id: 'pop-fix', type: 'success', position: { x: 350, y: 260 }, data: { label: 'Proof-of-Possession', sublabel: 'Token bound to agent\'s key', icon: '🔑' } },
      // Annotations
      { id: 'ann-bearer', type: 'annotation', position: { x: 0, y: 140 }, data: { label: 'OAuth: whoever holds the token can use it. If the letter is stolen, the thief has full access.', variant: 'danger' } },
      { id: 'ann-pop', type: 'annotation', position: { x: 0, y: 270 }, data: { label: 'Auth51: every request signed with the agent\'s private key. Stolen tokens are cryptographic noise.', variant: 'success' } },
      { id: 'next', type: 'check', position: { x: 700, y: 270 }, data: { label: 'Next →', icon: '▶', clickable: true } },
    ],
    edges: [
...
function getAct3(): { nodes: Node[]; edges: Edge[] } {
  return {
    nodes: [
      { id: 'studio', type: 'actor', position: { x: 0, y: 80 }, data: { label: 'Studio', sublabel: 'Organization', icon: '🏢' } },
      { id: 'producer', type: 'actor', position: { x: 220, y: 80 }, data: { label: 'Producer', sublabel: 'Orchestrator', icon: '🎬' } },
      { id: 'coordinator', type: 'actor', position: { x: 440, y: 80 }, data: { label: 'Coordinator', sublabel: 'Sub-agent', icon: '👤' } },
      { id: 'assistant', type: 'actor', position: { x: 660, y: 80 }, data: { label: 'Assistant', sublabel: 'Sub-sub-agent', icon: '👤' } },
      { id: 'vendor', type: 'actor', position: { x: 660, y: 220 }, data: { label: 'Vendor', sublabel: 'Resource', icon: '🏪' } },
      { id: 'scope', type: 'success', position: { x: 220, y: 220 }, data: { label: 'Scope narrows at each step', sublabel: 'Full → Catering → Single order', icon: '🔒' } },
      { id: 'ann-chain', type: 'annotation', position: { x: 0, y: 320 }, data: { label: 'Auth51 maintains the full delegation chain cryptographically. Every link is signed, scoped, and auditable back to the original authorization.', variant: 'brand' } },
      { id: 'next', type: 'check', position: { x: 660, y: 330 }, data: { label: 'What can go wrong? →', icon: '⚠️', clickable: true } },
    ],
    edges: [
...
function getAct4Hub(): { nodes: Node[]; edges: Edge[] } {
  return {
    nodes: [
      { id: 'center', type: 'check', position: { x: 320, y: 0 }, data: { label: 'What can go wrong?', icon: '⚠️' } },
      // Four clickable threat nodes
      { id: 'threat-fraudulent-vendor', type: 'threat', position: { x: 0, y: 120 }, data: { label: 'Fraudulent Vendor', sublabel: 'Privilege escalation', icon: '🏪', clickable: true } },
      { id: 'threat-rogue-assistant', type: 'threat', position: { x: 260, y: 120 }, data: { label: 'Rogue Assistant', sublabel: 'Scope escape', icon: '🎭', clickable: true } },
      { id: 'threat-stolen-letter', type: 'threat', position: { x: 500, y: 120 }, data: { label: 'Stolen Letter', sublabel: 'Token replay', icon: '📄', clickable: true } },
      { id: 'threat-substituted-producer', type: 'threat', position: { x: 720, y: 120 }, data: { label: 'Impersonation', sublabel: 'Identity spoofing', icon: '🎬', clickable: true } },
      { id: 'ann-choose', type: 'annotation', position: { x: 280, y: 260 }, data: { label: 'Click any threat to see how the flow breaks — and how Auth51 prevents it.', variant: 'info' } },
      { id: 'next', type: 'check', position: { x: 720, y: 270 }, data: { label: 'Skip to translation →', icon: '▶', clickable: true } },
    ],
    edges: [
...
function getThreatBranch(branch: ThreatBranch): { nodes: Node[]; edges: Edge[] } {
  ...
  return {
    nodes: [
      { id: 'attacker', type: 'threat', position: { x: 0, y: 100 }, data: { label: c.attacker, icon: c.attackerIcon } },
      { id: 'target', type: 'actor', position: { x: 250, y: 100 }, data: { label: 'Target', sublabel: 'Verification point', icon: '🎯' } },
      { id: 'fork', type: 'check', position: { x: 480, y: 100 }, data: { label: 'Auth check?', icon: '🔀' } },
      // Failure path (top)
      { id: 'no-auth51', type: 'threat', position: { x: 680, y: 10 }, data: { label: 'Without Auth51', sublabel: 'Bearer token only', icon: '✗' } },
      { id: 'fail-outcome', type: 'annotation', position: { x: 680, y: 100 }, data: { label: `${c.failLabel}\n${c.failDetail}`, variant: 'danger' } },
      // Success path (bottom)
      { id: 'with-auth51', type: 'success', position: { x: 680, y: 200 }, data: { label: 'With Auth51', sublabel: 'PoP + Scope + Identity', icon: '✓' } },
      { id: 'win-outcome', type: 'annotation', position: { x: 680, y: 290 }, data: { label: `${c.winLabel}\n${c.winDetail}`, variant: 'success' } },
      // Anchor reference
      { id: 'anchor', type: 'annotation', position: { x: 250, y: 280 }, data: { label: `Auth51 anchor: ${c.anchor}`, variant: 'brand' } },
      // Back button
      { id: 'back', type: 'check', position: { x: 0, y: 10 }, data: { label: '← Back', icon: '◀', clickable: true } },
    ],
    edges: [
...
function getAct5(): { nodes: Node[]; edges: Edge[] } {
  return {
    nodes: [
      { id: 'authority', type: 'actor', position: { x: 0, y: 80 }, data: { label: 'Auth51 Authority', sublabel: 'Control Plane (HA)', icon: '🏢' } },
      { id: 'runtime', type: 'artifact', position: { x: 260, y: 80 }, data: { label: 'Auth51 Runtime', sublabel: 'In-process library', icon: '⚙️' } },
      { id: 'jwt', type: 'artifact', position: { x: 500, y: 80 }, data: { label: 'Agentic JWT', sublabel: 'Intent Token + PoP', icon: '📜' } },
      { id: 'verifier', type: 'success', position: { x: 720, y: 80 }, data: { label: 'Auth51 Verifier', sublabel: 'Sidecar / Gateway', icon: '🛡️' } },
      { id: 'console', type: 'actor', position: { x: 0, y: 230 }, data: { label: 'Auth51 Console', sublabel: 'Operator UI', icon: '📊' } },
      { id: 'cli', type: 'actor', position: { x: 260, y: 230 }, data: { label: 'Auth51 CLI (a51)', sublabel: 'Operator tool', icon: '⌨️' } },
      { id: 'ann-flow', type: 'annotation', position: { x: 500, y: 230 }, data: { label: 'Agent loads Runtime → registers with Authority → signs with Agentic JWT → verified at Verifier. Operators observe via Console.', variant: 'brand' } },
      { id: 'next', type: 'check', position: { x: 720, y: 240 }, data: { label: 'Try it live →', icon: '🚀', clickable: true } },
    ],
    edges: [
...
function getAct6(): { nodes: Node[]; edges: Edge[] } {
  ...
  return {
    nodes: [
      ...
      { id: 'demo', type: 'annotation', position: { x: 720, y: 240 }, data: { label: '🚀 Live demo coming soon. Connect to a real Auth51 Authority and watch agent registration, token minting, and verification in real time.', variant: 'brand' } },
    ],
    edges: base.edges,
  }
}
