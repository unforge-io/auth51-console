...
export default function ThreatModel() {
  return (
    <article>
      ...
      <Lead>
        A conventional OAuth threat model assumes a fixed client. Once the client is an agent — code
        whose behavior is driven by a prompt, that spawns sub-agents and picks tools at runtime — a
        set of attacks opens up that the classic model doesn&rsquo;t name. The protocol&rsquo;s threat analysis
        enumerates twelve of them, using STRIDE as the frame.
      </Lead>
      <P>
        They&rsquo;re worth reading not as a checklist but as the reason each mechanism exists. Every anchor
        on the next page traces back to one or more of these.
      </P>
      ...
      <P>
        <strong>Spoofing (3).</strong> Pretending to be a trusted agent — by cloning its identity, by
        replaying a token it was issued, or by swapping in a compromised client library. All three
        attack the question &ldquo;is this really who it claims to be?&rdquo;
      </P>
      <P>
        <strong>Tampering (3).</strong> Changing what the agent is or does after it was trusted —
        modifying its prompt or tools at runtime, injecting instructions through its inputs, or
        rewriting the workflow definition it&rsquo;s supposed to follow.
      </P>
      <P>
        <strong>Elevation of privilege (3).</strong> Ending up able to do more than intended — one
        agent steering a higher-privileged one, skipping an approval step, or minting a broader scope
        than the step calls for.
      </P>
      <P>
        <strong>Repudiation (2).</strong> Breaking the link between a user&rsquo;s intent and an agent&rsquo;s
        action — forging the origin of an intent, or manipulating a delegation chain to hide who
        actually acted.
      </P>
      <P>
        <strong>Information disclosure (1).</strong> Leaking the agent&rsquo;s own configuration — its
        prompts and tools — which is both sensitive and a map for the spoofing attacks above.
      </P>
      ...
      <Callout>
        ...
        10 (prompt injection, excessive agency). The agent threats aren&rsquo;t exotic — they&rsquo;re familiar
        failure modes with a new actor in the client seat.
      </Callout>
