  ...
  return (
    <article>
      ...
      <P>
        ...
        prerequisites complete, gate passed, scope in bounds — before it issues a token. So the plan
        isn&rsquo;t enforced abstractly; it&rsquo;s enforced against the run&rsquo;s <em>actual</em> position, tracked
      ...
      <Deep title="How the tracking is injected, and why it’s language-agnostic">
        <P>
          ...
          effect. The technique is deliberately generic: it&rsquo;s wrapper functions in Python, the same idea
          via interceptors or aspects elsewhere, so nothing about the tracking is tied to one framework.
        </P>
        <P className="!mb-0">
          Tracking can be turned off — at the cost of losing the workflow-level guarantees — which keeps
          adoption incremental. There&rsquo;s a real tradeoff: it adds a little in-memory bookkeeping per
          call, with no external round-trip, so the runtime overhead is small.{' '}
